...
CMD ["/opt/rclone/rclone" , "serve" , "${RCLONE_BACKEND_PROTOCOL}" , "${RCLONE_BACKEND_OPTIONS}" , "--user" ,"${RCLONE_BACKEND_USER}" , "--pass" , "${RCLONE_BACKEND_PASS}" , "--addr" , "${RCLONE_BACKEND_ADDR}" , "${RCLONE_BACKEND}:${RCLONE_BACKEND_PATH}" ]
